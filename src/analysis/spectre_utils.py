...
def auc_stats(graph_ref_list, graph_pred_list, score_fn):

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    test_dataset = BZR(root='your/data/dir', dataset_name='bzr', split='test')
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)
    model = MyGNNModel(*model_args, **model_kwargs).to(device)
    checkpoint = torch.load('path/to/your_model.pth', map_location=device)
    model.load_state_dict(checkpoint)
    model.eval()

    def compute_anomaly_score_nn(model, batch):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            scores = compute_anomaly_score_nn(model, batch)
...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(
        ...
        if "auc" in self.metrics_list:
            if local_rank == 0:
                print("Computing AUC stats...")
            auc = auc_stats(reference_graphs, networkx_graphs, score_fn=max_degree_score)
            to_log["auc"] = auc
            if wandb.run:
...
class SamplingMetrics(SpectreSamplingMetrics):
