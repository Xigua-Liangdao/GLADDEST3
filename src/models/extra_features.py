...
class ExtraFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        X_extra, E_extra, y_graph = None, None, None

        # Logic for each feature type to determine X, E, and graph-level y
        if self.features_type == "cycles":
            X_extra = x_cycles
            E_extra = torch.zeros((*E_t.shape[:-1], 0)).type_as(E_t)
            y_graph = torch.hstack((n, y_cycles))
            ...
            y_graph = torch.hstack((n, y_cycles, n_components, batched_eigenvalues))

        elif self.features_type == "rrwp":
            E = E_t.float()[..., 1:].sum(-1)
            rrwp_edge_attr = self.RRWP(E, k=self.rrwp_steps)
            diag_index = torch.arange(rrwp_edge_attr.shape[1], device=E.device)
            X_extra = rrwp_edge_attr[:, diag_index, diag_index, :]
            E_extra = rrwp_edge_attr
            y_graph = torch.hstack((n, y_cycles))
            ...
            max_value = rrwp_edge_attr_wo_norm.max(dim=1, keepdim=True).values.max(dim=2, keepdim=True).values
            rrwp_edge_attr_wo_norm = rrwp_edge_attr_wo_norm / max_value
            E_extra = torch.cat((rrwp_edge_attr, rrwp_edge_attr_wo_norm), dim=-1)
            diag_index = torch.arange(E_extra.shape[1], device=E.device)
            X_extra = E_extra[:, diag_index, diag_index, :]
            y_graph = torch.hstack((n, y_cycles))
            ...
            X_extra = torch.cat((rrwp_node_attr, comp_rrwp_node_attr), dim=-1)
            E_extra = torch.cat((rrwp_edge_attr, comp_rrwp_edge_attr), dim=-1)
            y_graph = torch.hstack((n, y_cycles))
            ...
            X_extra = torch.cat((x_cycles, nonlcc_indicator, k_lowest_eigvec), dim=-1)
            y_graph = torch.hstack((n, y_cycles, n_components, batched_eigenvalues))

        else:
            raise ValueError(f"Features type {self.features_type} not implemented")

        # Final expansion of y to node-level
        node_mask = noisy_data['node_mask']
        n_max = node_mask.shape[1]
        y_node = y_graph.unsqueeze(1).expand(-1, n_max, -1)
        y_node = y_node * node_mask.unsqueeze(-1)

        return utils.PlaceHolder(X=X_extra, E=E_extra, y=y_node)
